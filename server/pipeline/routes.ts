...
export function registerPipelineRoutes(app: Express) {
   ...
   *
   * Today's NBA games with live scores from ESPN (free, no key required).
   * Returns STATUS_SCHEDULED, STATUS_IN_PROGRESS, and STATUS_FINAL games for today.
   */
  app.get("/api/nba/scoreboard", async (_req: Request, res: Response) => {
    try {
      const today = new Date().toISOString().slice(0, 10).replace(/-/g, "");
      const url = `https://site.api.espn.com/apis/site/v2/sports/basketball/nba/scoreboard?dates=${today}`;
      const resp = await fetch(url);
      if (!resp.ok) {
        return res.status(502).json({ error: `ESPN scoreboard returned ${resp.status}` });
      }
      ...
      const games = (data.events ?? []).map((event: any) => {
        ...
        return {
          ...
          statusDesc: st?.description ?? "Scheduled",
          statusType: st?.name ?? "STATUS_SCHEDULED",
          ...
          overUnder: odds?.overUnder ? `O/U ${odds.overUnder}` : null,
          broadcast,
        };
      });
