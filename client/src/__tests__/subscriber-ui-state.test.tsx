...
describe("subscriber-aware UI state", () => {
  ...
  it("shows active Pro subscriber state on My Edge without upgrade CTAs", () => {
    ...
    expect(document.body.textContent ?? "").toContain("NBA Watchlist slot");
    expect(document.body.textContent ?? "").toContain("MLB Watchlist slot");
    expect(document.body.textContent ?? "").toContain("What changed: Limited participation");
    expect(document.body.textContent ?? "").toContain("Why it matters: Team rotation");
    expect(document.body.textContent ?? "").toContain("What changed: A lineup");
    expect(document.body.textContent ?? "").toContain("Why it matters: Team plans");
    expect(document.body.textContent ?? "").not.toMatch(/(?:NBA|MLB)Watchlist slot/);
    expect(document.body.textContent ?? "").not.toMatch(/What changed[A-Z]/);
    expect(document.body.textContent ?? "").not.toMatch(/Why it matters[A-Z]/);
    expect(document.body.textContent ?? "").toContain("Followed player availability will surface here first");
    expect(document.body.textContent ?? "").toContain("Followed-team lineup changes will appear in your watchlist");
    expect(document.body.textContent ?? "").not.toContain("Followed-player availability would move fantasy");
    expect(document.body.textContent ?? "").not.toContain("Followed-team lineup change would trigger");
