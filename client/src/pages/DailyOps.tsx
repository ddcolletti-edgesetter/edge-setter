...
export default function DailyOps({ theme, toggleTheme }: Props) {
  ...
  const { data: latest, isLoading: latestLoading } = useQuery<any>({
    ...
    queryFn: () => apiRequest("GET", "/api/agent/daily-ops/latest?password=" + ADMIN_PASSWORD).then(r => r.json()),
  ...
  const { data: history = [], isLoading: historyLoading } = useQuery<any[]>({
    ...
    queryFn: () => apiRequest("GET", "/api/agent/daily-ops?password=" + ADMIN_PASSWORD + "&limit=30").then(r => r.json()),
    refetchInterval: 60000,
  });
  ...
  const runMutation = useMutation({
    mutationFn: (sendEmail: boolean) =>
      apiRequest("POST", "/api/agent/daily-ops/run", { password: ADMIN_PASSWORD, send_email: sendEmail }).then(r => r.json()),
    onSuccess: (data) => {
